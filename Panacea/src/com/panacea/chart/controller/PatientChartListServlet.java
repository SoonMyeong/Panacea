 ...
 */
@WebServlet("/chart/patientChartList")
public class PatientChartListServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public PatientChartListServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			list = new ChartService().selectPatient(userId,cPage,numPerPage);
		...
		try {
			totalChartCount = new ChartService().patientChartCount(userId);
		} catch (ChartException e) {
			e.printStackTrace();
			throw new ServletException("차트 오류");
		}
		...
		//2.3 페이지바구성
				...
				if(pageNo == 1 ){
					...
					pageBar += "<a href='"+request.getContextPath()+"/chart/patientChartList?cPage="+(pageNo-1)+"&userId="+userId+"'>[이전]</a> ";
				}
...
//				while(pageNo<=pageEnd && pageNo<=totalPage){
				while(!(pageNo>pageEnd || pageNo > totalPage)){
					...
					if(cPage == pageNo ){
						...
						pageBar += "<a href='"+request.getContextPath()+"/chart/patientChartList?cPage="+pageNo+"&userId="+userId+"'>"+pageNo+"</a> ";
					}
					pageNo++;
				}
				...
				if(pageNo > totalPage){
					...
					pageBar += "<a href='"+request.getContextPath()+"/chart/patientChartList?cPage="+pageNo+"&userId="+userId+"'>[다음]</a>";
				}
		...
		request.getRequestDispatcher("/WEB-INF/views/chart/patientChartList.jsp").forward(request, response);
