...
public class NoticeDeleteServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String[] noticeNo2 = noticeNo.split(",");
		//2.서비스로직호출
		int result=0;
		
		for(int i=0; i<noticeNo2.length; i++) {
			try {
				result = new NoticeService().deleteNotice(Integer.parseInt(noticeNo2[i]));
			} catch (NumberFormatException | NoticeException e) {
				e.printStackTrace();
