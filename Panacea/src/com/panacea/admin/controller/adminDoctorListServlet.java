...
/**
 * Servlet implementation class adminDoctorListServlet
 */
...
public class adminDoctorListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			list = new AdminService().selectDoctorList(cPage,numPerPage);
		} catch (AdminException e) {
			e.printStackTrace();
			throw new ServletException("관리자 오류");
		}
		System.out.println("list@doctorListServlet="+list);
		
		
		int totalPatientCount = 0;
		try {
			totalPatientCount = new AdminService().doctorListCount();
		} catch (AdminException e) {
			e.printStackTrace();
			throw new ServletException("관리자 오류");
		}
		int totalPage = (int)Math.ceil((double)totalPatientCount/numPerPage);
		System.out.println("totalDoctorListCount="+totalPatientCount+", totalPage="+totalPage);
		...
		if(pageNo == 1 ){
			...
			pageBar += "<a href='"+request.getContextPath()+"/admin/doctorList?cPage="+(pageNo-1)+"'>[이전]</a> ";
		}
...
//		while(pageNo<=pageEnd && pageNo<=totalPage){
		while(!(pageNo>pageEnd || pageNo > totalPage)){
			...
			if(cPage == pageNo ){
				...
				pageBar += "<a href='"+request.getContextPath()+"/admin/doctorList?cPage="+pageNo+"'>"+pageNo+"</a> ";
			}
			pageNo++;
		}
		...
		if(pageNo > totalPage){
			...
			pageBar += "<a href='"+request.getContextPath()+"/admin/doctorList?cPage="+pageNo+"'>[다음]</a>";
		}
